...
public sealed class AuthService : IAuthService
{
    ...
    public async Task<LoginCommandResponse> CreateTokenByRefreshTokenAsync(CreateNewTokenByRefreshTokenCommand request, CancellationToken cancellationToken)
    {
        ...
        if (user.RefreshTokenExpires < DateTime.Now)
            throw new Exception("Refresh tekenun süresi dolmuş!");
    ...
    {
        User? user = await _userManager.Users.Where(p => p.UserName == request.UserNameOrEmail || p.Email == request.UserNameOrEmail).FirstOrDefaultAsync(cancellationToken);
